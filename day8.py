...
# 2. Write the INSERT string with placeholders (?)
insert_sql = """
INSERT INTO properties (building_name, price, gla) 
VALUES (?, ?, ?)
"""
    
for property in master_database:

    # 3. Create a tuple with the actual data
    real_data = (property["name"], property["price"], property["gla"])

    # 4. Execute the command, combining the string and the data
    cursor.execute(insert_sql, real_data)
